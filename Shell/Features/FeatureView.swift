    ...
    private var summaryCard: some View {
        HStack(spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text(AppLocalization.labeledCount("cylinders.active.label", count: store.activeCylinders.count, locale: locale)).font(.headline)
                                ...
                                NavigationLink { SupplierDetail(store: store, supplier: supplier, isEntitled: isEntitled, requestUpgrade: requestUpgrade) } label: {
                                    HStack {
                                        ...
                                        VStack(alignment: .leading) {
                                            ...
                                            Text(AppLocalization.labeledCount("cylinders.current.label", count: count, locale: locale)).font(.subheadline).foregroundStyle(.secondary)
                                        }
